...
var connect = function (conn, callback) {
  ...
  this.connection.on('error', function (err) {
    console.log('MySQL Database error occured:\r\n', err);
    if (err.code === 'PROTOCOL_CONNECTION_LOST') {
      console.log('MySQL database connection lost, reconnecting...')
      rootObj.connect(myConn, connCallback)
    } else {
      throw err;
    }
  });
...
var get = (table, options, callback) => {
  ...
  if (!joinAll && Array.isArray(options.select)) {
    ...
    options.select.forEach((el) => {
      select += `,${(options.join && el[0] != ' ') ? table.name + '.' : ''}${el}`;
    })
  ...
  for (var indx of options.join) {
    ...
    if (Array.isArray(indx.select)) {
      indx.select.forEach((el) => {
        select += `,${el[0] != ' ' ? (indx.as || indx.with) + '.' : ''}${el}`;
      })
    }
    else
